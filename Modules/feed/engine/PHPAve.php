...
// Fixed Interval with summing

class PHPAve
{
    private $dir = "/var/lib/phpave/";
    private $log;
    ...
    */
	...
	private function getVariable($id, $key) {
		...
		$this->log->info("PHPAve:getVariable $key  -->  $value");
		return $value;
	}
	
	private function setVariable($id, $key, $value) {
		$className = get_class($this);
		$key = "$className.$id.$key";
		$this->log->info("PHPAve:setVariable $key  -->  $value");
		$memcache = new Memcached();
		$memcache->addServer("localhost", 11211);
...
// #### \/ Below are required methods
    ...
    */
    public function post($id,$timestamp,$value,$padding_mode=null)
    {
		...
		$value = $this->getVariable($id, "sum") + $value;
		$this->setVariable($id, "sum", $value);
		
		$tmp_last_pos = $this->getVariable($id, "last_pos");
		$this->setVariable($id, "last_pos", $last_pos);

        $cnt = $this->getVariable($id, "cnt") + 1;
		$this->setVariable($id, "cnt", $cnt);
		
		$value = $value/$cnt;
		
        if ($tmp_last_pos!=$last_pos) {
			$this->setVariable($id, "sum", 0);
			$this->setVariable($id, "cnt", 0);
        }
        ...
        // Close file
        fclose($fh);
         $this->log->info( "PHPAVE: post ret $value");
    ...
    */
    public function get_data($name,$start,$end,$interval,$skipmissing,$limitinterval)
    {
        ...
        while($time<=$end)
        {
			...
			$sum = 0;
			$cnt = 0;
			while($time<=$time2 && $time<=$end)
			{
				$time = $start + ($meta->interval * $i);
    ...
    public function export($id,$start)
    {
        ...
        // If meta data file does not exist exit
        if (!$meta = $this->get_meta($id)) {
            $this->log->warn("PHPAve:post failed to fetch meta id=$id");
            return false;
        }
    ...
    public function post_bulk_save()
    {
        ...
        foreach ($this->writebuffer as $feedid=>$data) {
            ...
            if (@filesize($filename)%4 != 0) {
                ...
                fwrite($fh,$data);
                fclose($fh);
                print "PHPAVE: FIXED DATAFILE WITH INCORRECT LENGHT '$filename'\n";
